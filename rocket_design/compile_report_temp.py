...
def markdown_to_docx_elements(doc, markdown_content):
    ...
    # Simple parsing approach - look for patterns in the markdown
    ...
    while i < len(lines):
        ...
        if line.startswith('#'):
            ...
            if level <= 3:  # Only supporting up to h3
                text = line.lstrip('#').strip()
                heading = doc.add_heading(text, level=level)
                heading.style = f'Custom Heading {level}'
...
def main():
    ...
    # Changed output filename to avoid permission issues
    output_doc = "Rocket_Design_Report_Updated.docx"
